...
function _populate() {
    if (!_populated) {
        ...
        if (_iphone = /\b(iPhone|iP[ao]d)/.exec(t), _ipad = /\b(iP[ao]d)/.exec(t), _android = /Android/i.exec(t), _native = /FBAN\/\w+;/i.exec(t), _mobile = /Mobile/i.exec(t), _win64 = !!/Win64/.exec(t), e) {
            ...
            _ie_real_version = s ? parseFloat(s[1]) + 4 : _ie, _firefox = e[2] ? parseFloat(e[2]) : NaN, _opera = e[3] ? parseFloat(e[3]) : NaN, (_webkit = e[4] ? parseFloat(e[4]) : NaN) ? (e = /(?:Chrome\/(\d+\.\d+))/.exec(t), _chrome = e && e[1] ? parseFloat(e[1]) : NaN) : _chrome = NaN
...
class ClapatSlider {
    constructor(t, e = {}) {
        ...
        this.opts = Object.assign({
            ...
            tsSnap: null,
            events: []
        }, this.init(), ClapatSlider.instances.push(this)
    }
    bindAll() {
    ...
    setup() {
        ...
        for (let i = t.length - 1; i >= 0; i--) {
            const s = t[i],
                n = s.cloneNode(!0);
            "vertical" == this.opts.direction ? (s.style.top = 100 * i + "%", n.style.top = 100 * -e + "%", n.classList.add(this.opts.clones), this.elInner.append(n)) : (s.style.left = 100 * i + "%", n.style.left = 100 * -e + "%", n.classList.add(this.opts.clones), this.elInner.prepend(n)), e++
        }
        if (null != this.elPagination)
            for (let e = 0; e < t.length; e++) {
                t[e];
                let i = document.createElement("div");
                i.classList.add("clapat-pagination-bullet"), "function" == typeof this.opts.renderBullet && (i.innerHTML = this.opts.renderBullet()), this.elPagination.appendChild(i), i.addEventListener("click", this.onPagination), this.itemsPagination.push({
                    el: i
                })
            }
        ...
        for (let t = 0; t < i.length; t++) {
            ...
            "vertical" == this.opts.direction ? (o = {
                ...
                length: h,
                translate: 0
            }, this.length += h), e.classList.contains(this.opts.clones) ? (this.itemsCloned.push(o), o.clone = !0) : (this.itemsInitial.push(o), o.clone = !1), this.items.push(o)
        }
    ...
    updateUI() {
        ...
        null != this.el && (this.state.currentScrollDirection >= 0 ? (this.el.classList.contains("bw") && this.el.classList.remove("bw"), this.el.classList.add("fw")) : (this.el.classList.contains("fw") && this.el.classList.remove("fw"), this.el.classList.add("bw")));
    ...
    snapTargetOnWheel(t, e) {
        ...
        for (let n = 0; n < this.items.length; n++) {
            ...
            if (Math.abs(c) < l.length / 2) {
                return a == n && Math.abs(e - t) < this.length ? (this.log("Snapping - start and end items are the same, the scroll did not go over a full item"), this.state.currentScrollDirection > 0 ? c < 0 ? e - (l.length / 2 + c + l.nextElement.length / 2) : e - c : c < 0 ? e - c : e - c + l.prevElement.length) : (this.log("Snapping - start and end items are different"), e - c)
            }
        }
        return e
    }
    ...
    onMove(t) {
        ...
        Math.abs(a) > Math.abs(h) && t.cancelable && t.preventDefault(), "vertical" == this.opts.direction ? (e.target = n + h * this.opts.speed, e.currentScrollDirection = -Math.sign(h)) : (e.target = n + a * this.opts.speed, e.currentScrollDirection = -Math.sign(a))
    }
    onUp(t) {
        if (!this.enabled) return;
        this.log("on up");
    ...
    onWheel(t) {
        ...
        i.target -= s * this.opts.speed, i.currentScrollDirection = Math.sign(s), this.opts.snap && this.snapWheelEvents.events.push({
            ts: e,
            delta: s,
            currentTarget: i.target
        }), "vertical" == this.opts.direction ? h.y = i.target : h.x = i.target, i.off = i.target
    }
    updateWheelSnap() {
